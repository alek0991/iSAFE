...
2. ```bcftools``` version 1.2 or above. Please follow the [bcftools installation guideline](http://www.htslib.org/download/). You can also try the following alternative way to install bcftools:
    ...
    ```
    - In case you follow bcftools installation guideline, make sure you do all the steps, because iSAFE assumes the bcftools binary file is installed to a bin subdirectory that is added to your ```$PATH```. Otherwise, you have to change the following line in ```./src/bcftools.py``` to the bcftools binary file path: 
    ```sh
    bcftools = "bcftools"
    ```
Data Requirements
